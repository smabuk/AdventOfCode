...
public static class LineIntersection
{
	...
	internal static DecimalPoint? FindIntersection(Line lineA, Line lineB, decimal tolerance)
	{
		...
		if (x1 == x2 && x3 == x4 && x1 == x3) {
			...
			if (IsInsideLine(lineA, firstIntersection) &&
				IsInsideLine(lineB, firstIntersection)) {
		...
		// x,y can intersect outside the line segment since line is infinitely long
		// so finally check if x, y is within both the line segments
		if (IsInsideLine(lineA, result) &&
			IsInsideLine(lineB, result)) {
			return result;
		}
	...
	/// </summary>
	private static bool IsInsideLine(Line line, DecimalPoint p)
	{
		decimal x = p.X, y = p.Y;
...
public record class Line
{
	...
	internal Line(DecimalPoint start, DecimalPoint end) : this()
	{
		if (start.X < end.X) {
			Left  = start;
			Right = end;
